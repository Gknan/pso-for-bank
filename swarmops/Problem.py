...
from data_tools import ATMData # 计算公式所在的位置
...
class BankProblem(Benchmark):
    """
    The BankProblem benchmark problem.
    data: 传入的参数 实际预测的七天加钞数据
    """
    def __init__(self, dim, data):
        Benchmark.__init__(self, name='BankProgramm', dim=dim, fitness_min=0.0,
                           lower_bound=-50.0, upper_bound=50.0,
                           lower_init=5.0, upper_init=50.0)
        self.data = data

    # x 的输入是一维的，需要转为 2 维后输入计算
