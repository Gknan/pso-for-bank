...
# C:\Users\Gknan\Documents\WeChat Files\wxid_5952499525012\FileStorage\File\2020-03\python_root\python_root\DATA
    ...
    dim = data.ATM_number  # # dimension of particles, which is the number of variables of func
    day = data.days  # 第三个维度
    ...
    problem = Problem.BankProblem(dim=dim * day, data=data)
    ...
    if True:
        ...
        # Control parameters.
        parameters = PSO.parameters_100dim_200000eval # [161.0, -0.2089, -0.0787, 3.7637]
...
########################################################################
    ...
    results = MultiRun(optimizer=optimizer, parameters=parameters,
                       ...
                       parallel=parallel,
                       max_evaluations=max_evaluations,
                       display_interval=display_interval, trace_len=trace_len, max_add_data = data.max_add_money)
